    ...
    while (1) {
        ...
        if (length < 0) {
            ...
            switch (msgtype) {
                ...
                {
                    //IR Sensor
                    if (msgbuffer[0] == I2C_IR_ADDRESS || msgbuffer[0] == I2C_MOTOR_ADDRESS){
                        unsigned char *msg = msgbuffer+1;
                        //0 means no data is available and 0xFF means that there is an error (No connection`)
                        if (msg[2] != 0 && msg[2] != 0xFF)
                            start_UART_send(8, msg);
                    }
                    else if (msgbuffer[0] == COLOR_ADDRESS) {
                        // Dhiraj, do stuff here. The message comes in the form:
                        //   msgbuffer[1] = register address, [2] = register value
                        // so you could do something like this
                        // if (msgbuffer[1] == DATA_RED_LO) do something
                        // or a switch statment if you prefer
                    }
                };
                case MSGT_I2C_DBG:
                {
